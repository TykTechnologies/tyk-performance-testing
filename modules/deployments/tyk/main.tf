...
locals {
  ...
  # POLICIES_POLICYPATH being non-empty routes the gateway's policy loader
  # (gateway/server.go) into LoadPoliciesFromDir(PolicyPath), which globs
  # *.json in that directory and decodes each file as one user.Policy - see
  # api-definitions.tf's policy_definitions for the corresponding one-
  # file-per-policy ConfigMap shape and why the "_id" field must be omitted.
